...
restore_atari_state,stable_normalizer,smooth,symmetric_remove,Database, sample_discrete)
...
def agent(game,n_ep,n_mcts,max_ep_len,lr,c,gamma,data_size,batch_size,temp,n_hidden_layers,n_hidden_units,v_tau,p_tau,epsilon):
    ...
    for ep in range(n_ep):
        ...
        mcts = MCTS(root_index=s, root=None, model=model, na=Env.info.action_space.n, gamma=gamma, v_tau=v_tau,
                    p_tau=p_tau, epsilon=epsilon)  # the object responsible for MCTS searches
